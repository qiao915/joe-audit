...
export async function auditPackage(projectRoot, savePath) {
  ...
  console.log(`Starting audit: ${projectRoot}`);
  console.log(`Report will be saved to: ${finalSavePath}\n`);
  
  // 创建进度条实例
  const progressBar = new cliProgress.SingleBar({
    format: 'Progress: [{bar}] {percentage}% | Stage: {stage} | {message}',
    barCompleteChar: '█',
    barIncompleteChar: '░',
    hideCursor: true,
    stopOnComplete: true
  ...
  progressBar.start(totalSteps, 0, {
    stage: 'Preparing',
    message: 'Initializing audit process'
  });
  ...
  try {
    ...
    progressBar.update(currentStep, {
      stage: 'Step 1/7',
      message: 'Creating temporary working directory'
    });
    workDir = await createWorkDir();
    
    // 2. 解析项目，向工作目录添加package.json
    currentStep++;
    const step2Message = 'Parsing project structure and dependencies';
    progressBar.update(currentStep, {
      stage: 'Step 2/7',
      message: step2Message
    });
    const stopAnimation2 = startSpinAnimation('Step 2/7', step2Message);
    ...
    // 3. 生成依赖锁定文件
    currentStep++;
    const step3Message = 'Generating dependency lock file';
    progressBar.update(currentStep, {
      stage: 'Step 3/7',
      message: step3Message
    });
    const stopAnimation3 = startSpinAnimation('Step 3/7', step3Message);
    ...
    // 4. 对工作目录进行审计
    currentStep++;
    const step4Message = 'Executing security audit';
    progressBar.update(currentStep, {
      stage: 'Step 4/7',
      message: step4Message
    });
    const stopAnimation4 = startSpinAnimation('Step 4/7', step4Message);
    ...
    console.log(`\n✅ Audit completed!`);
    console.log(`Report saved to: ${finalSavePath}`);
    ...
    if (workDir) {
      try {
        // 确保临时目录被清理，即使中间步骤出错
        await deleteWorkDir(workDir);
        console.log('✅ Temporary working directory cleaned up');
      } catch (cleanupError) {
        // 清理失败时不影响主流程
        console.warn(`Warning: Failed to clean up temporary working directory ${workDir}: ${cleanupError.message}`);
      }
    }
  }
}
